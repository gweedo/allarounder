...
- **Health verification constraint — backend is internal-only:** the backend Container App has `external: false` ingress; its FQDN is only routable within the Container Apps environment and is unreachable from GitHub Actions runners. Green-revision health for the backend is validated by the Container Apps **readiness probe** (configured in Bicep), which must pass before the platform marks the revision `Running`. The CI workflow treats `Running` state as the health gate and skips any external `curl`. The **frontend** has `external: true` and is the only app with a publicly curl-able FQDN; frontend smoke tests and Playwright E2E run against it after traffic is flipped.
