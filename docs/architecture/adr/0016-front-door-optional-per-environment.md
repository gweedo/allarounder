...
The v1 infra cost-optimization pass (issue #71, following `retrospective-infra-cost-overprovisioning-2026-06-25.md` action item #4) reviewed the live Azure footprint for MVP-appropriate sizing. Azure Front Door Standard carries a flat ~€31/month base fee **per environment**, independent of traffic (ADR-0015).
...
- **Keep Front Door on both environments (status quo).** Simplest, but pays ~€31/month for TLS/redirect/WAF capability staging structurally cannot exercise the way production does (no custom domain, no real traffic pattern to rate-limit against).
...
- **Cheaper:** ~€31/month saved on staging (issue #72's estimate), no functional loss for production.
